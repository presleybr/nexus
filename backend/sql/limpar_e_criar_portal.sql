-- ============================================
-- PORTAL CONSÓRCIO - LIMPEZA E CRIAÇÃO COMPLETA
-- Database: nexus_crm (porta 5434)
-- ============================================

-- ============================================
-- PASSO 1: DROPAR TABELAS EXISTENTES (na ordem correta)
-- ============================================

DROP TABLE IF EXISTS historico_disparos CASCADE;
DROP TABLE IF EXISTS pastas_digitais CASCADE;
DROP TABLE IF EXISTS boletos CASCADE;
DROP TABLE IF EXISTS clientes_finais CASCADE;
DROP TABLE IF EXISTS usuarios_portal CASCADE;
DROP TABLE IF EXISTS configuracoes_automacao CASCADE;
...
CREATE INDEX idx_clientes_finais_nexus ON clientes_finais(cliente_nexus_id);
CREATE INDEX idx_clientes_finais_cpf ON clientes_finais(cpf);
CREATE INDEX idx_clientes_finais_status ON clientes_finais(status_contrato);

CREATE INDEX idx_boletos_cliente_final ON boletos(cliente_final_id);
CREATE INDEX idx_boletos_vencimento ON boletos(data_vencimento);
CREATE INDEX idx_boletos_status ON boletos(status);
CREATE INDEX idx_boletos_status_envio ON boletos(status_envio);

CREATE INDEX idx_pastas_cliente_final ON pastas_digitais(cliente_final_id);
...
-- Configuração padrão para cliente ID 2
...
-- Verificar contagens
SELECT
    (SELECT COUNT(*) FROM usuarios_portal) as usuarios,
    (SELECT COUNT(*) FROM clientes_finais) as clientes,
    (SELECT COUNT(*) FROM configuracoes_automacao) as configs;
